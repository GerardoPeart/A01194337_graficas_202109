...
function main() 
{
    ...
    let ronbo = createRombo(gl);
    let pac = createPac(gl);

    mat4.identity(modelViewMatrix);
    
    mat4.translate(modelViewMatrix, modelViewMatrix, [squareo.posx,squareo.posy,squareo.posz]);
    ...
    mat4.translate(modelViewMatrix, modelViewMatrix, [triangleo.posx, triangleo.posy, triangleo.posz]);
    ...
    mat4.translate(modelViewMatrix, modelViewMatrix, [romboo.posx, romboo.posy, romboo.posz]);

    bindShaderAttributes(gl, shaderProgram);
    draw(gl, shaderProgram, ronbo);

    mat4.identity(modelViewMatrix);
    
    mat4.translate(modelViewMatrix, modelViewMatrix, [1.6, -1.5, -7]);

    bindShaderAttributes(gl, shaderProgram);
    draw(gl, shaderProgram, pac);
}
...
function createSquare(gl) 
{
  //hacer el triangulo 
    ...
    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(squareo.verts), gl.STATIC_DRAW);

    let square = {buffer:vertexBuffer, vertSize:squareo.vertsize, nVerts:squareo.vertn, primtype:gl.TRIANGLE_STRIP};
...
function createRombo(gl) 
{
  //hacer el rombo
    ...
    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(romboo.verts), gl.STATIC_DRAW);

    let Ronbo = {buffer:vertexBuffer, vertSize:romboo.vertsize, nVerts:romboo.vertn, primtype:gl.TRIANGLE_STRIP};
...
function createTriangle(gl)
{
  //haccer el triangulo
    ...
    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(triangleo.verts), gl.STATIC_DRAW);
    
    let triangle = {buffer:vertexBuffer, vertSize:triangleo.vertsize, nVerts:triangleo.vertn, primtype:gl.TRIANGLES};
    return triangle;
} 
...
function createPac(gl) 
{
  ...
  //hacer un circlulo y cortarlo hasta un grado 
    let vertexBuffer;
    vertexBuffer = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, vertexBuffer);
    ...
    for (var i = 0.0 ; i<=360 ; i+=1){
        ...
        var vert1 = [-Math.sin(j-1), Math.cos(j-1)];

        var vert2 = [0,0];

        verts = verts.concat(vert1);
        verts = verts.concat(vert2);
    }


    gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(verts), gl.STATIC_DRAW);

    let Pac = {buffer:vertexBuffer, vertSize:2, nVerts:560, primtype:gl.TRIANGLE_STRIP};
